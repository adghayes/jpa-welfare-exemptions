...
def parse_all_staff_reports(pdf_dir: str = "data/pdfs") -> list[StaffReportGrant]:
    """
    Parse all Staff Report PDFs in a directory.

    Args:
        pdf_dir: Directory containing the PDFs

    Returns:
        List of all StaffReportGrant objects
    """
    pdf_path = Path(pdf_dir)
    all_grants = []

    # Find all staff report PDFs (handle both - and _ separators)
    staff_files = sorted(list(pdf_path.glob("*-staff_report.pdf")) +
                         list(pdf_path.glob("*-staff-report.pdf")))

    print(f"Found {len(staff_files)} staff report PDFs")

    for pdf_file in staff_files:
        print(f"Parsing: {pdf_file.name}")
        grants = parse_staff_report_pdf(pdf_file)
        print(f"  Found {len(grants)} grants")
        all_grants.extend(grants)

    print(f"\nTotal grants from staff reports: {len(all_grants)}")
    return all_grants


if __name__ == "__main__":
    grants = parse_all_staff_reports()
    for g in grants[:5]:  # Show first 5
        print(f"  {g.property_name} - {g.applicant}")
        print(f"    {g.city}, {g.county}")
        print(f"    Units: {g.total_units} total, {g.restricted_units} restricted")
